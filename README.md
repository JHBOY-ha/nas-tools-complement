...
新增手动上传字幕能力，并将“展示媒体、标记中文字幕状态、手动上传字幕”的组件独立为“媒体整理 → 媒体库”页面。该功能适用于自动字幕下载失败、存量媒体需要人工补全字幕，或需要按媒体库分类批量巡检中文字幕状态的场景。
...
| `web/templates/rename/medialibrary.html` | 新增独立媒体库页面，展示已同步媒体并提供上传字幕入口 |
| `web/static/js/media-library.js` | 抽离媒体库列表、筛选、分页、剧集选择与字幕上传交互 |
| `web/static/js/media-sync.js` | 抽离媒体库同步弹窗逻辑，供首页复用 |
| `web/static/components/layout/navbar/index.js` | 在“媒体整理”分类下新增“媒体库”菜单入口 |
| `web/main.py` | 新增登录保护的字幕上传接口 |
| `app/subtitle.py` | 新增手动字幕保存、命名、冲突编号和目标同步逻辑 |
| `app/helper/db_helper.py` | 新增按源文件查询最新整理历史 |
| `app/mediaserver/media_server.py` | 支持按指定 Emby/Jellyfin 刷新媒体库 |
| `app/library.py` | 新增媒体库展示、分类、字幕状态检测、剧集查询与封面兜底逻辑 |
| `tests/test_media_library.py` | 新增媒体库展示、分类、字幕检测与封面兜底回归测试 |
| `tests/test_subtitle_upload.py` | 新增手动字幕上传回归测试 |

**功能改进：**

1. 在“媒体整理 → 媒体库”中独立展示已同步媒体，支持按类型、小分类、字幕状态和关键词筛选。
2. 媒体卡片会标记中文字幕状态：已有外挂中文字幕、已有内嵌中文字幕、缺中文字幕或未检测。
3. 电影可直接上传字幕；电视剧/动漫可先选择具体剧集，再上传对应剧集字幕。
4. 在“媒体整理 → 文件管理”选中电影后，仍可在“下载字幕”旁点击“上传字幕”。
5. 支持上传 `.srt`、`.ass`、`.ssa` 单个字幕文件。
6. 上传时可选择目标影视服务器：Jellyfin 或 Emby，默认优先选择 Jellyfin。
7. 字幕会优先保存到媒体链接后的目标媒体文件目录；若有整理历史，会根据历史中的目标路径定位媒体库文件。
8. 若没有整理历史或目标路径不可访问，则仅对可访问的媒体服务器路径启用上传；不可访问的卡片会禁用上传按钮。
9. 目标字幕使用 Emby/Jellyfin 可识别的外挂字幕命名，例如：
   - `Movie (2024).zh-CN.srt`
   - `Movie (2024).zh-TW.ass`
   - `Movie (2024).eng.srt`
10. 语言会从上传字幕文件名自动识别，无法识别时默认按简体中文 `zh-CN` 处理。
11. 已存在同名字幕时不会覆盖，会自动追加编号，例如 `Movie (2024).zh-CN(1).srt`。
12. 同步到媒体库目标目录时会参考整理历史中的转移模式：硬链接使用硬链接，软链接使用软链接，复制/移动类使用复制，避免删除源目录字幕。
13. 上传完成后会尝试刷新所选 Emby/Jellyfin 媒体库；刷新失败不会回滚字幕保存，会在返回消息中提示。
14. 封面加载采用分页和懒加载策略：独立页面默认每页展示较少媒体，首批封面优先加载，其余封面进入视口附近后再拉取，降低 NAS 和媒体服务器瞬时压力。
