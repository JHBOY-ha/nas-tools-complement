...
选择结果后点击“下载并保存”；ZIP/RAR 字幕包会先列出文件供选择，剧集只列出明确匹配当前季集的字幕，无法确认或属于其他集的文件不能保存。RAR 需要安装 Python 依赖 `rarfile` 及系统工具 `unrar` 或 `unar`。单次下载和字幕包展开内容限制为 20 MB。下载后进入统一字幕任务队列，可在任务中心查看进度；后台完成格式与编码校验、保存到选中的媒体文件旁，并更新字幕状态和刷新媒体服务器。保留已有字幕，沿用任务设置中的队列、暂存和资源限制。下载结果有效期为 30 分钟，过期后重新搜索。媒体文件需位于已配置的媒体库目录内且可读取。
