...
在“媒体整理 → 文件管理”中为单部电影新增手动上传字幕入口，适用于自动字幕下载失败或需要人工补全指定影片字幕的场景。
...
1. 在“媒体整理 → 文件管理”选中电影后，可在“下载字幕”旁点击“上传字幕”。
2. 支持上传 `.srt`、`.ass`、`.ssa` 单个字幕文件。
3. 上传时可选择目标影视服务器：Jellyfin 或 Emby，默认优先选择 Jellyfin。
4. 字幕会先保存到原始电影文件所在目录，再根据最新整理历史同步到媒体库目标电影目录。
5. 若没有整理历史，可在弹窗中手动选择目标电影文件；不选择时仅保存源目录字幕。
6. 目标字幕使用 Emby/Jellyfin 可识别的外挂字幕命名，例如：
   - `Movie (2024).zh-CN.srt`
   - `Movie (2024).zh-TW.ass`
   - `Movie (2024).eng.srt`
7. 语言会从上传字幕文件名自动识别，无法识别时默认按简体中文 `zh-CN` 处理。
8. 已存在同名字幕时不会覆盖，会自动追加编号，例如 `Movie (2024).zh-CN(1).srt`。
9. 同步到媒体库目标目录时会参考整理历史中的转移模式：硬链接使用硬链接，软链接使用软链接，复制/移动类使用复制，避免删除源目录字幕。
10. 上传完成后会尝试刷新所选 Emby/Jellyfin 媒体库；刷新失败不会回滚字幕保存，会在返回消息中提示。
