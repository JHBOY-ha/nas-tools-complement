...
4. 新增检索增强上下文：在 LLM 解析前可先检索 TMDB与Bangumi 候选，并作为 `external_candidates` 提供给模型参考。
5. 增强检索 query 归一化（去平台/编码/发布组噪声，提取核心标题），提高复杂标题命中率。
6. 支持 LLM 返回 `tmdb_id/tmdb_type`，命中时优先按 ID 拉取 TMDB 详情，失败自动回退到原名称检索。
7. 新增稳定性兜底：LLM 超时、异常、非法 JSON 时不影响原流程，自动回落规则识别。
8. 新增可观测性：日志增加 LLM 原始返回、检索候选数量、直出 TMDBID 记录，便于排查识别问题。
9. 配置迁移兼容旧版本：旧 `config.yaml` 自动补全 `llm` 字段。

</details>

<details>
<summary><b>9. Python 升级后本地 .venv 重建与兼容性验证</b></summary>

在 macOS 本地完成 Python 升级后的虚拟环境重建，并验证核心运行链路。

**修改文件：**

| 文件 | 变更说明 |
|:---|:---|
| `README.md` | 新增 Python 升级后 `.venv` 重建与验证记录 |

**功能改进：**

1. 删除旧 `.venv` 后使用 Python 3.11 重建虚拟环境，并重新安装依赖。
2. 验证 `openai` 与 `typing_extensions.override` 可正常导入，确认不再出现 `ImportError`。
3. 本地环境固定 `httpx 0.27.x` 与 `setuptools < 81`，避免 OpenAI SDK `proxies` 参数不兼容和 `pkg_resources` 缺失问题。
4. 完成两轮启动冒烟（首次启动 + 重启启动），服务可正常拉起且无导入类异常。
5. 已知限制：当前本地 `config/config.yaml` 未配置 TMDB API Key，相关 TMDB 用例在无密钥条件下无法完整通过。
