...
<details>
<summary><b>7. 识别流程修复：忽略片名尾部版本号 + 中文兜底检索</b></summary>
...
<details>
<summary><b>8. LLM 媒体识别增强</b></summary>
...
<details>
<summary><b>9. Docker 镜像更新与部署规范</b></summary>
...
<details>
<summary><b>10. 新增微信(OpenClaw)消息推送接口</b></summary>
...
<details>
<summary><b>11. 新增手动上传字幕功能并同步媒体库目标目录</b></summary>

新增手动上传外挂字幕、同步媒体链接目标目录、字幕时间轴对齐、媒体服务器可识别性检测和问题字幕二次处理能力。“媒体整理 → 字幕库”只展示已经进行过媒体链接/整理的影视文件，适用于人工补全字幕、排查 Jellyfin/Emby/Plex 无法识别的外挂字幕，以及按媒体库分类分批检查存量字幕。
...
| `app/helper/subtitle_align.py` | 基于目标视频内嵌文本字幕进行整体偏移、多锚点分段和 LLM 跨语言对齐 |
| `app/utils/llm_client.py` | 统一 OpenAI Chat Completions 兼容请求，支持 API 根地址及完整端点容错 |
| `app/helper/db_helper.py` | 新增按源文件查询最新整理历史 |
| `app/mediaserver/media_server.py` | 支持按选定 Emby/Jellyfin/Plex 类型刷新媒体库 |
...
3. 筛选和排序条件收纳到统一菜单，点击“应用筛选”后一次加载；配合分页、封面懒加载和目录状态缓存，降低短时间 NAS 扫描压力。
4. 媒体卡片会同时展示内嵌/外挂中文字幕状态和最近一次外挂字幕检测结果。电视剧/动漫可先选择具体剧集再上传。
5. 支持单次上传最多 20 个 `.srt`、`.ass`、`.ssa`、`.smi`、`.vtt`、`.sub` 字幕文件。文件会先进行编码、内容结构和字幕格式校验，必要时转换为 UTF-8 并修复可安全处理的 SRT/ASS 问题。
6. 上传时可选择 Jellyfin、Emby 或 Plex；默认使用全局影视服务器配置。
7. 字幕首先保存到当前媒体文件同目录；存在可用的整理历史且目标文件不同时，再按整理模式将字幕硬链接、软链接或复制到媒体库目标目录，不删除源字幕。
8. 目标字幕使用 Jellyfin/Emby/Plex 可识别的语言标签、来源和序号命名，例如：
   - `Movie (2024).zh-CN.srt`
   - `Movie (2024).zh-TW.ass`
   - `Movie (2024).zh-CN.bluray(1).srt`
9. 已存在同名字幕时不会覆盖。同一电影的同语种多来源字幕会保留来源标记或追加序号，避免二次处理时相互覆盖。
10. 可选字幕对齐模式：不对齐、自动选择、仅整体偏移、多锚点分段、LLM 跨语言对齐。对齐功能使用目标视频内嵌文本字幕作为参考，当前仅对 SRT/VTT 执行时间轴改写；低置信度、缺少 ffmpeg/ffprobe 或无可用参考轨时会安全跳过。
11. LLM 跨语言对齐为显式选择的可选功能，默认上传不调用 API。仅支持 OpenAI Chat Completions 兼容协议；`Base URL` 推荐填写 API 根地址（如 `https://openrouter.ai/api/v1`），也容错接受完整的 `/chat/completions` 地址，不支持 Anthropic Messages 格式。
12. “外挂字幕全部检测”按全局影视服务器规则检查字幕是否可读取、语言标签和命名是否规范。必须选择电影/电视剧/动漫分类，并可继续选择国产电影、外国电影、动画电影等小分类，避免每次扫描整个媒体库。
13. 电影卡片保留最近一次检测状态，检测窗口保留最近 3 次分类检测记录。检测结果区分“通过”、“语言标签/命名不规范”、“无法识别”和“扫描错误”。
14. 对于可读取但语言显示“未定义”、命名不规范或编码/格式异常的电影字幕，卡片提供“二次处理”；处理后只复检当前电影并刷新状态，不重新扫描整个分类。
15. 上传或二次处理完成后会尝试刷新目标媒体服务器。刷新失败不会回滚已成功保存的字幕，会在返回消息中提示。
...
> 建议将容器 `/config` 挂载到你当前正在使用的宿主机配置目录，避免历史配置和数据库丢失。
> 如不使用代理，可删除 `HTTP_PROXY`、`HTTPS_PROXY`、`NO_PROXY` 三项。
...
电影卡片点击“在线字幕”，电视剧和动漫先“选择剧集”，按季、集筛选后点击对应集的“检索本集字幕”。检索框默认填入媒体名、英文原名（如有）、年份及季集，可手动修改或删除关键词，按回车或点击“检索字幕”搜索，也可点击“恢复默认词”。编辑后的检索词会原样传给迅雷或 Assrt（剧集未填写季集时自动补齐目标季集）；剧集以独立季集参数检索，并将下载结果绑定到选中的视频路径。检索结果会校验片名（含原名）、已知年份和季集，过滤无法确认的结果；缺少年份的电影结果会单独标注。迅雷无需配置，优先展示与本地视频 CID 匹配的字幕。Assrt 的 Token 在“设置 → 字幕 → 在线字幕”保存，也可配置 `subtitle.assrt.token`，不影响原有自动字幕下载器。

选择结果后点击“下载并保存”；ZIP/RAR 字幕包会先列出文件供选择，剧集只列出明确匹配当前季集的字幕，无法确认或属于其他集的文件不能保存。RAR 需要安装 Python 依赖 `rarfile` 及系统工具 `unrar` 或 `unar`。单次下载和字幕包展开内容限制为 20 MB。字幕保存到选中的媒体文件旁，经过现有格式与编码校验，保留已有字幕并刷新媒体服务器。下载结果有效期为 30 分钟，过期后重新搜索。媒体文件需位于已配置的媒体库目录内且可读取。
