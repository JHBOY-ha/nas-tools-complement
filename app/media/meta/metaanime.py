            ...
            if anitopy_info:
                ...
                if self.cn_name:
                    ...
                    if self.cn_name:
                        ...
                        self.cn_name = self.normalize_release_version_suffix(self.cn_name)
                ...
                if self.en_name:
                    ...
                    self.en_name = self.normalize_release_version_suffix(self.en_name)
                ...
                if not anime_season and name:
                    # 检查标题是否以数字结尾（如 "Mato Seihei no Slave 2"）
                    season_from_name = re.search(r'^(.+?)\s+(\d{1,2})$', name.strip())
                    if season_from_name:
                        potential_season = season_from_name.group(2)
                        # 确保这个数字确实是季数（标题后紧跟 " - 集数" 格式）
                        if re.search(r'\s+%s\s*-\s*\d{1,4}' % potential_season, title):
                            anime_season = potential_season
                            # 从标题中移除季数
                            name = season_from_name.group(1).strip()
                            # 更新英文名
                            if self.en_name and self.en_name.strip().endswith(potential_season):
                                self.en_name = re.sub(r'\s+%s$' % potential_season, '', self.en_name).strip()
                                self._name = StringUtils.str_title(self.en_name)
                            # 更新中文名
                            if self.cn_name and self.cn_name.strip().endswith(potential_season):
                                self.cn_name = re.sub(r'\s+%s$' % potential_season, '', self.cn_name).strip()
