...
class Media:
    ...
    def __search_tmdb(self, file_media_name,
                      ...
                      media_year=None,
                      season_number=None,
                      language=None, anime_only=False):
        """
        ...
        if search_type == MediaType.MOVIE:
            ...
            if media_year and season_number:
                ...
                info = self.__search_tv_by_season(file_media_name,
                                                  media_year,
                                                  season_number, anime_only=anime_only)
            if not info:
                log.debug(
                ...
                info = self.__search_tv_by_name(file_media_name,
                                                first_media_year, anime_only=anime_only)
            if info:
    ...
    def __search_tv_by_name(self, file_media_name, first_media_year, anime_only=False):
        """
        根据名称查询电视剧TMDB匹配
        ...
        if anime_only:
            tvs = [tv for tv in tvs if 16 in (tv.get("genre_ids") or
                   [g.get("id") for g in tv.get("genres", [])])]
        if len(tvs) == 0:
    ...
    def __search_tv_by_season(self, file_media_name, media_year, season_number, anime_only=False):
        """
    ...
    def get_cache_info(self, meta_info):
        ...
        if (cached.get("id") and self.__resolve_tmdb_mtype(meta_type=cached.get("type"))
            ...
            return {}
        if (cached.get("id") and meta_info.type == MediaType.ANIME
                and cached.get("type") != MediaType.ANIME):
            return {}
        return cached

    @staticmethod
    def __is_same_search_name(name1, name2):
        """
        比较两个检索名称是否相同（忽略大小写和特殊字符）
        """
        if not name1 or not name2:
            return False
        return StringUtils.handler_special_chars(name1).upper() == StringUtils.handler_special_chars(name2).upper()

    @staticmethod
    def __extract_cn_fallback_name(meta_info):
        """
        提取中文兜底检索名
        """
        def _clean_candidate(text):
            ...
            # 去掉末尾集号标记（例如 - 06、第06集）
            text = re.sub(r"(?i)\s*[-_~]?\s*(?:第?\s*\d{1,4}\s*[集话話期]?)\s*$", "", text)
            text = re.sub(r"(?i)\s*[-_~]?\s*\d{1,4}(?:[vV]\d{1,2})?\s*$", "", text)
        ...
        def _is_valid_cn_candidate(text):
            if not text:
                return False
            if not StringUtils.is_chinese(text):
                return False
            ...
            if zh_count / max(len(pure), 1) < 0.35:
                return False

            # 英文字符过多时容易是“中英混合+标签”串
            if len(re.findall(r"[A-Za-z]", text)) >= 8:
                return False

            # 排除技术标签和字幕说明
            if re.search(r"(?i)\b(?:HEVC|AVC|X26[45]|AAC|EAC3|FLAC|WEB[- ]?RIP|WEB[- ]?DL|BDRIP|BLURAY|REMUX|MKV|MP4|1080P|2160P|720P|10BIT|8BIT)\b", text):
                return False
            if re.search(r"字幕|内封|外挂|简繁|简体|繁体|中字|双语|中日|招募", text):
                return False
            return True
        ...
        # 优先使用已识别中文名（经清洗后再校验）
        cn_name = _clean_candidate(meta_info.cn_name or "")
        if _is_valid_cn_candidate(cn_name):
            candidates.append(cn_name)

        org_string = (meta_info.org_string or "").strip()
        if not org_string:
            return candidates[0] if candidates else None

        # 去掉开头发布组 [xxx]
        org_string = re.sub(r"^\s*\[[^\]]*]\s*", "", org_string).strip()

        # 1) 先从分隔段（/）中提取
        split_parts = re.split(r"\s*[／/]\s*", org_string)
        for part in split_parts:
            part = (part or "").strip()
            if not part:
                continue
            # 段内 []/【】内容作为候选
            for b1, b2 in re.findall(r"\[([^\]]+)\]|【([^】]+)】", part):
                block = _clean_candidate(b1 or b2)
                if _is_valid_cn_candidate(block):
                    candidates.append(block)
            # 去掉括号段后作为候选
            plain = _clean_candidate(re.sub(r"\[[^\]]*]\s*|【[^】]*】\s*", " ", part))
            if _is_valid_cn_candidate(plain):
                candidates.append(plain)

        # 2) 再从整串 []/【】内容兜底提取
        for b1, b2 in re.findall(r"\[([^\]]+)\]|【([^】]+)】", org_string):
            block = _clean_candidate(b1 or b2)
            if _is_valid_cn_candidate(block):
                candidates.append(block)
        ...
        for item in candidates:
            key = StringUtils.handler_special_chars(item).upper()
            if key in seen:
                continue
            seen.add(key)
    ...
    def __extract_llm_tmdb_target(self, meta_info, mtype_hint=None):
        ...
        if not llm_note.get("candidate_verified"):
            return None, None
    ...
    def __search_media_with_name(self, meta_info, query_name, strict=None, allow_aliases=True):
        ...
        """
        if not meta_info or not query_name:
            return {}

        if meta_info.type not in [MediaType.TV, MediaType.ANIME] and not meta_info.year:
            file_media_info = self.__search_multi_tmdb(file_media_name=query_name)
        else:
        ...
        if not file_media_info and self._search_tmdbweb:
            file_media_info = self.__search_tmdb_web(file_media_name=query_name, mtype=meta_info.type)

        if not file_media_info and self._search_keyword:
            cache_name = cacheman["tmdb_supply"].get(query_name)
            is_movie = False
            if not cache_name:
                cache_name, is_movie = self.__search_engine(query_name)
                cacheman["tmdb_supply"].set(query_name, cache_name)
            if cache_name:
    ...
    def get_media_info_on_files(self,
                                ...
                                chinese=True,
                                download_context=None, download_contexts=None):
        """
                ...
                # 自带TMDB信息
                    ...
                    if download_context and bound_type != MediaType.MOVIE:
                        ...
                        if file_episodes and episodes and not set(file_episodes).issubset(set(episodes)):
                            # 仅在TMDB完整给出之前各季集数、且映射后命中RSS集号时转换绝对集号。
                            counts = {entry.get("season_number"): entry.get("episode_count")
                                      for entry in (tmdb_info.get("seasons") or [])}
                            prior = list(range(1, seasons[0])) if len(seasons) == 1 else []
                            can_map = (not explicit_season and prior
                                       and all(isinstance(counts.get(s), int) and counts[s] > 0 for s in prior))
                            offset = sum(counts[s] for s in prior) if can_map else 0
                            mapped = [ep - offset for ep in file_episodes]
                            if not offset or not set(mapped).issubset(set(episodes)):
                                raise ValueError("文件集号与RSS下载任务冲突，需确认绝对集数映射：%s" % file_name)
                            meta_info.begin_episode = mapped[0]
                            meta_info.end_episode = mapped[-1] if len(mapped) > 1 else None
                            log.info("【Meta】按已验证的前季集数转换绝对集号：%s -> %s" % (file_episodes, mapped))
                        if (not file_episodes and len(file_list) == 1 and len(episodes) == 1
                                and download_context.get("allow_episode_fallback", True)):
