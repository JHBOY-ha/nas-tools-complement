...
class Emby(_IMediaClient):
    ...
    def get_item_image(self, item_id, image_type="Primary"):
        ...
        if not self._host or not self._apikey:
            return None
        req_url = "%semby/Items/%s/Images/%s?api_key=%s" % (self._host, item_id, image_type, self._apikey)
        try:
    ...
    def get_episodes(self, series_id):
        ...
        if not self._host or not self._apikey or not self._user:
            return []
        req_url = "%semby/Users/%s/Items?ParentId=%s&Recursive=true&IncludeItemTypes=Episode&Fields=Path,MediaStreams,ProviderIds&api_key=%s" % (
